...
def catalogo_view(request):
    ...
    inmuebles_sup_menos250 = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(surface__lt=250)
    )
    # Inmuebles con una frente mayor a 10m
    inmuebles_front_mas10 = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(front__gt=250)
    )
    # Inmueble con un fondo mayor a 16m
    inmuebles_bot_mas15 = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(bottom__gt=250)
    )
    # Inmueble con un precio menor a 20 mil persos por metro cuandrado
    inmuebles_precio_menos20k = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(price__lt=20000)
    )
    # Inmueble con una precio total menor a un millón de pesos
    inmuebles_Totprecio_menos1m = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(totalprice__lt=100000)
    )
    # Inmueble con con regimen proiedad privada
    inmuebles_rp_privada = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(regimen_propiedad__exact="Privada")
    )
    # inmueble con regimen proviedad ejidal
    inmuebles_rp_ejidal = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(regimen_propiedad__exact="Ejidal")
    )
    # Inmueble con regimen propiedad comunal
    inmuebles_rp_comunal = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(regimen_propiedad__exact="Comunal")
    )
    # Inmuebles con uso de suelo residencial
    inmuebles_us_residencial = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(uso_suelo__exact="Residencial")
    )
    # Inmuebles con uso de suelo actividades productivas
    inmuebles_us_ap = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(uso_suelo__exact="Actividades Productivas")
    )
    # Inmuebles con uso de suelo Equipamiento
    inmuebles_us_equipamiento = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(uso_suelo__exact="Equipamiento")
    )
    # Inmuebles con uso de suelo Infraestructura
    inmuebles_us_infraestructura = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(uso_suelo__exact="Infraestructura")
    )
    # Inmuebles con uso de suelo Area Verde
    inmuebles_us_averde = Inmueble.objects.filter(
        Q(status__exact="Oferta"), Q(uso_suelo__exact="Area Verde")
    )
    ...
    if querset:
        ...
        if not inmuebles:
            mensaje = (
                "Lo sentimos, no encontramos ningún inmuebles con esas características"
            )
    return render(
        request,
...
def me_interesa(request):
    ...
    if request.method == "POST":
        ...
        data = {"interesados": inmueble_obj.interesados.all().count()}
